...
namespace imap_copy {
    ...
    public:
        ...
        [[nodiscard]] auto listAllUids(const MailboxConfig &account,
                                       const std::string &folder) const -> std::vector<uint64_t>;

        [[nodiscard]] auto fetchMetaByUid(const MailboxConfig &account, const std::string &folder,
                                          uint64_t uid) const -> std::optional<MessageMeta>;

        [[nodiscard]] auto downloadMessageByUid(const MailboxConfig &account, const std::string &folder,
                                                uint64_t uid) const -> std::vector<char>;

        [[nodiscard]] auto appendMessage(const MailboxConfig &account, const std::string &folder,
        ...
        [[nodiscard]] auto clearSeenByMessageId(const MailboxConfig &account,
