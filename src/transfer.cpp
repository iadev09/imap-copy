...
namespace imap_copy {
    ...
    auto transferMessages(const AppConfig &cfg, bool delete_after_copy,
                          const size_t requested_worker_count) -> TransferStats {
        TransferStats stats;
            ...
            while (true) {
                ...
                try {
                    ...
                    if (!before_uids.empty()) {
                        dest_max_uid_before = *std::ranges::max_element(before_uids);
